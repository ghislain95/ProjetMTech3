...
#Valeurs idéales des sliders pour du vert fluo / bleu:
    #l_h = 30    
    #l_s = 40
    #l_v = 67
    #u_h = 98 / 104
...
def dessAccord(ac):
    for (corde,frette) in ac:
        corde -=1
        dessNote(frette,corde,gridXR,gridYR,img)


#Vecteur de pourcentages : M[i] = longueur Sillet-ième frette en pourcentage (1 = longueurDiapason)
def matFr():
...
def createGrid(x11,y11,x21,y21, M, longD):
    #crée un tableau allant de la valeur x21 à x11, en nb points
...
def createGridY(x11,y11,x21,y21, M, longD):
    #crée un tableau allant de la valeur y21 à y11, en 6 points
    x = np.linspace(int(x21), int(x11), nb)
    y = np.linspace(int(y21), int(y11), 6)
...
while True:
   ...
   #   -   -   -
    ...
    grid = createGrid(int(x11),int(y22+((y22-y21)/5)),int(x21),int(y21+((y22-y21)/8)), M, longDiapason)
    gridX = createGridX(int(x11),int(y22+((y22-y21)/5)),int(x21),int(y21+((y22-y21)/8)), M, longDiapason)
    gridY = createGridY(int(x11),int(y22+((y22-y21)/5)),int(x21),int(y21+((y22-y21)/8)), M, longDiapason)
    ...
    for x in range(0,nb):
        for y in range(0,6):
            ...
            #cv2.circle(img, (int(xG), int(yG)), 2, (0, 255, 0), -1)
